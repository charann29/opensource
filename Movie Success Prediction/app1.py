...
# Limit to 6 genres
limited_genres = dict(list(genres.items())[:6])

# Refit the label encoder with the limited genres
genre_names = list(limited_genres.values())
...
for idx, movie in enumerate(popular_movies[:6]):  # Display only the first 6 popular movies, 2 per row
    with cols[idx % 2]:
        ...
        genre_names = [limited_genres[genre_id] for genre_id in movie['genre_ids'] if genre_id in limited_genres]
